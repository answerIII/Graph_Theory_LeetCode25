...
def createUndirAdjList(input_file_path: str) -> List[Set[int]]:
    separator = None
    if (input_file_path[-4:] == ".csv"):
        separator = ","
    with open(Path(input_file_path), mode='r') as input_file:
        nodes, edges = map(int, input_file.readline().split(sep=separator))
        adj_list = [set() for _ in range(nodes)]
...
def createDirAdjList(input_file_path: str) -> List[Set[int]]:
    separator = None
    if (input_file_path[-4:] == ".csv"):
        separator = ","
    with open(Path(input_file_path), mode='r') as input_file:
