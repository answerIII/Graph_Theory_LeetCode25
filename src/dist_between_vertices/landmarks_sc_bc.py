...
def processFile(file_path: str) -> None:
    ...
    for landmarks_num in total_landmarks_lst:
        ...
        landmarks_lst = chooseBestCoverageNodes(
        ...
        for _ in range(TOTAL_DIST_COMPUTES):
            ...
            bc_lm_sc_start_time = time()
            bc_dist_sum += landmarksSC(
                lwc_undir_adj_list,
                spt_lst,
                landmarks_lst,
                landmarks_num,
            ...
            bc_lm_sc_time += time() - bc_lm_sc_start_time
        ...
        print(
            ...
            f"best coverage nodes: {bc_lm_sc_time} sec"
        )
        print(f"\t\tTotal: {time() - total_start_time} sec")
        print("\tAccuracy")
        print(
            "\t\tLandmarks-SC best coverage landmarks: "
            f"{real_dist_sum / bc_dist_sum}"
        )
