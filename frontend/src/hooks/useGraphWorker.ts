...
export function useGraphWorker() {
  ...
  const processFile = useCallback(async (file: File, directed: string, isVeryLargeGraph: boolean, removeLoops: boolean) => {
    ...
    return new Promise<GraphUpload>((resolve, reject) => {
      ...
      worker.postMessage({ file, is_directed: directed, isVeryLargeGraph, removeLoops });
    }).finally(() => {
      setLoading(false);
      worker.terminate();
    });
